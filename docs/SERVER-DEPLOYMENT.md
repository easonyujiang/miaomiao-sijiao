...
```bash
# 1. 克隆代码
git clone https://github.com/easonyujiang/miaomiao-sijiao.git
cd miaomiao-sijiao/server
...
```ini
[Unit]
Description=妙喵私教 API Server
After=network.target

[Service]
Type=simple
User=www-data
WorkingDirectory=/opt/miaomiao-sijiao/server
ExecStart=/opt/miaomiao-sijiao/server/.venv/bin/uvicorn ewa.api.main:app --host 0.0.0.0 --port 8000 --workers 2
Restart=always
RestartSec=5
Environment=PATH=/opt/miaomiao-sijiao/server/.venv/bin
EnvironmentFile=/opt/miaomiao-sijiao/server/.env
...
sudo systemctl enable miaomiao
sudo systemctl start miaomiao
sudo systemctl status miaomiao
```
...
# crontab -e
0 3 * * * sqlite3 /opt/miaomiao-sijiao/server/data/miaomiao.db ".backup '/opt/backups/miaomiao_$(date +\%Y\%m\%d).db'"
```
...
```bash
sudo chown -R www-data:www-data /opt/miaomiao-sijiao/server/data
```
